...
test('idle narrative with long file path renders single-line (v0.5.3 path truncation)', () => {
  ...
  // `C:\Dev\sample-app\src\clients\page.tsx` rendered untruncated
  ...
  const enriched = enrichedFixture({
    ...
    primaryFiles: ['C:/Users/demo/Dev/sample-app/src/clients/dashboard/page.tsx'],
    pathClusters: { 'C:/Users/demo/Dev/sample-app/src/clients/dashboard': 12 },
    events: [
  ...
  assert.doesNotMatch(
    recap.narrative,
    /C:\/Users\/demo\/Dev\/sample-app\/src\/clients\/dashboard\/page\.tsx/,
  );
...
test("stuck: no 'try again' tone claim when the user is not correcting (v0.8.1)", () => {
  // The stuck-soft and refuse_to_verify paths fire with a neutral or idle
  // user — pre-fix the narrative asserted conversational pushback that
  // never happened.
  for (const tone of ['neutral', 'idle']) {
    const recap = renderRecap(
      enrichedFixture(),
      outcomeFixture({ verificationTrend: 'flat_fail', userToneTrend: tone }),
      verdictFixture({ bucket: 'stuck', confidence: 0.7 })
    );
    assert.doesNotMatch(
      recap.narrative,
      /try again/i,
      `must not claim a 'try again' tone for userToneTrend=${tone}`
    );
    assert.match(recap.narrative, /Worth checking in/);
  }
});

test('done: "finished up about X ago" anchors on the assistant sign-off (v0.8.1)', () => {
  // idleGapMs (user-message anchor) is 15 minutes; completionGapMs
  // (assistant sign-off anchor) is 30 seconds. Pre-fix the narrative said
  // "finished up about 15 minutes ago" the moment the verb landed.
  const recap = renderRecap(
    enrichedFixture(),
    outcomeFixture({
      completionVerbsRecent: true,
      idleGapMs: 15 * 60_000,
      completionGapMs: 30_000,
    }),
    verdictFixture({ bucket: 'done', confidence: 0.85 })
  );
  assert.match(recap.narrative, /finished up about \*\*30 seconds\*\* ago/);
  assert.doesNotMatch(recap.narrative, /15 minutes/);
});

test('done: falls back to idleGapMs when completionGapMs is absent (v0.8.1)', () => {
  const recap = renderRecap(
    enrichedFixture(),
    outcomeFixture({ completionVerbsRecent: true, idleGapMs: 5 * 60_000 }),
    verdictFixture({ bucket: 'done', confidence: 0.85 })
  );
  assert.match(recap.narrative, /finished up about \*\*5 minutes\*\* ago/);
});
