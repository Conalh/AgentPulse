...
test('path clusters: strip per-event cwd so absolute paths cluster by relative subdirs', () => {
  const cwd = 'C:/Dev/sample-app';
  const events = [
    toolUse(100, 'Edit', { file_path: 'C:/Dev/sample-app/app.py' }, cwd),
    toolUse(200, 'Edit', { file_path: 'C:/Dev/sample-app/utils/foo.py' }, cwd),
    toolUse(300, 'Edit', { file_path: 'C:/Dev/sample-app/utils/bar.py' }, cwd),
    toolUse(400, 'Read', { file_path: 'C:/Dev/sample-app/src/api/x.py' }, cwd),
...
test('path clusters: backslash cwd on Windows still strips correctly', () => {
  const cwd = 'C:\\Dev\\sample-app';
  const events = [
    toolUse(100, 'Edit', { file_path: 'C:\\Dev\\sample-app\\utils\\foo.py' }, cwd),
    toolUse(200, 'Edit', { file_path: 'C:\\Dev\\sample-app\\utils\\bar.py' }, cwd),
  ];
  const e = enrichWindow(events, WSTART, WEND);
  assert.equal(e.pathClusters['utils'], 2);
});
