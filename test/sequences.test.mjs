...
test('analyzeSequences: multiple edits sharing ONE failing verify count as ONE cycle (v0.8.1)', () => {
  // Pre-fix, detectStuckLoop never advanced past a matched verification,
  // so every edit before a single failing test run paired with that same
  // run: Edit(a) → Edit(a) → Bash(npm test, fail) counted as TWO failing
  // cycles and fired stuck_loop — the first iteration of a normal TDD
  // session classified as stuck (and gated `--once --strict`). A cycle is
  // one edit-run → one verify-run.
  const events = [
    toolUse(FRESH + 1000, 'Edit', { file_path: 'src/auth.ts' }),
    toolUse(FRESH + 2000, 'Edit', { file_path: 'src/auth.ts' }),
    bashEvent(FRESH + 3000, 'npm test', { exitCode: 1, resultText: 'FAIL auth.test.ts' }),
  ];
  const sig = analyzeSequences(events);
...
test('analyzeSequences: stuck_loop still fires when each failing verify is its own run (v0.8.1)', () => {
  // Companion to the shared-verify test above: three edits, two SEPARATE
  // failing runs — a genuine loop. The consume-the-verify fix must not
  // weaken real detection.
  const events = [
    toolUse(FRESH + 1000, 'Edit', { file_path: 'src/auth.ts' }),
    toolUse(FRESH + 1500, 'Edit', { file_path: 'src/auth.ts' }),
    bashEvent(FRESH + 2000, 'npm test', { exitCode: 1, resultText: 'FAIL auth.test.ts' }),
    toolUse(FRESH + 3000, 'Edit', { file_path: 'src/auth.ts' }),
    bashEvent(FRESH + 4000, 'npm test', { exitCode: 1, resultText: 'FAIL auth.test.ts' }),
  ];
