 ...
 *    `C:\\Dev\\sample-app\\src\\clients\\page.tsx` before going idle."
...
function renderStuck(enriched: EnrichedWindow, outcome: OutcomeSignal): string {
  ...
  // v0.8.1: only claim a 'try again' tone when the user is actually
  // correcting. The stuck-soft and refuse_to_verify paths fire with a
  // neutral or idle user — pre-fix the narrative asserted conversational
  // pushback that never happened.
  const tone =
    outcome.userToneTrend === 'correcting'
      ? " The conversation has a 'try again' tone."
      : '';
  return sentence + tone + ' Worth checking in.';
}

function renderDone(enriched: EnrichedWindow, outcome: OutcomeSignal): string {
  // v0.8.1: anchor "finished up about X ago" on the last ASSISTANT message
  // (the sign-off), not the last user message. Pre-fix, a user prompt at T
  // followed by 15 minutes of agent work and a "done" at T+15m rendered as
  // "finished up about 15 minutes ago" the moment the verb landed.
  const idleAgo = humanDuration(outcome.completionGapMs ?? outcome.idleGapMs);
  const editCount = enriched.actionCounts.editing ?? 0;
