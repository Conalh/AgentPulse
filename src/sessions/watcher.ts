...
export function createSessionWatcher(opts: WatcherOptions = {}): SessionWatcher {
  ...
  function tryRecursiveWatch(root: ResolvedRoot): boolean {
    // Node 24's Windows recursive fs.watch can abort inside libuv for
    // newly-created nested paths, before JavaScript can catch an error.
    // Use the existing polling fallback there until the runtime path is safe.
    if (isWin32 && Number(process.versions.node.split('.')[0] ?? 0) >= 24) {
      return false;
    }
