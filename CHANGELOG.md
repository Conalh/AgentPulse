...
### Changed — public release and hosted-output privacy

- The composite Action now runs on Node 24, matching the package's Node 22+
  engine requirement after the Ink 7 upgrade. It builds and runs the checked-out
  Action source and lockfile instead of resolving a second copy from npm.
...
### Fixed — `stuck_loop` double-counted a single failing test run

`detectStuckLoop` never advanced past a matched verification, so every edit before one failing test run paired with that same run: `Edit(a) → Edit(a) → npm test (fail)` counted as TWO failing cycles and fired `stuck_loop` (threshold ≥2) — the first iteration of a perfectly normal TDD session classified as `stuck` at 0.85 confidence, and `--once --strict` gated on it. The detector now consumes the verification per cycle, exactly like `countEditVerifyCycles` always did: one edit-run → one verify-run is one cycle. Genuine loops (each failing verify its own run) still fire.

### Fixed — zero-count test summaries classified as failures

The text-only pass/fail tables (consulted only when no exit code is present — the common case on real transcripts) matched `\d+`, including zero. Green output that *mentions* an empty failure tally — `Tests: 0 failed, 5 passed` (jest/vitest), `Found 0 errors. Watching for file changes.` (tsc watch success), `0 failing` — classified as `fail`, and because failure dominates, fed `flat_fail` into the verification trend and biased the stuck rules. Count patterns now require a nonzero count, and bare `failing`/`passing` carry a not-preceded-by-`0` guard (mocha's `0 passing (2ms)` no-tests-ran output is no longer a pass either).

### Fixed — quoted URLs defeated the shell-exfil detector

`SHELL_EXFIL_RE` excluded quote characters between the fetch verb and the pipe (to keep `gh release create --notes "…curl…|sh…"` from false-positiving), which meant the single most common install one-liner shape — `curl -fsSL "https://get.docker.com" | sh` — was silently missed. Detection is now tokenizer-first: agent-gov-core's quote-aware `tokenizeShell` splits subcommands, `getCommandHead` resolves the executed verb through `sudo`/env wrappers, and the separator between adjacent spans is checked to be exactly one `|`. Quoted URLs are caught; the `gh --notes` false positive stays fixed; `&&`/`||` chains remain out of this rule family (documented). The regex survives only as a fallback when span reconstruction fails.

### Fixed — `..` traversal and `~/` writes bypassed the outside-repo detector

The repoRoot comparison was a raw `startsWith`, so `C:/dev/repo/../other/x.ts` — and a relative `../other/x.ts` resolved against cwd — read as *inside* the root. And `~/secrets.txt` was treated as relative, resolved to `<cwd>/~/…`, and passed too, meaning supplying a repoRoot actually *removed* the home-dir coverage the legacy prefix rule provides without one. `.`/`..` segments are now resolved lexically before comparison and `~`-prefixed paths count as outside. Symlink escapes are explicitly out of scope (README note).

### Fixed — narrative claims and tone reads that overstated what happened

Four smaller honesty fixes from the same review:

- **`renderStuck` asserted pushback that never happened.** The stuck narrative unconditionally appended "The conversation has a 'try again' tone." — but the stuck-soft and refuse-to-verify paths fire with a neutral or idle user. The sentence is now gated on `userToneTrend === 'correcting'`.
- **"Finished up about X ago" was anchored on the wrong message.** `idleGapMs` measures from the last USER message, so a prompt at T followed by 15 minutes of agent work and a "done" at T+15m rendered as "finished up about 15 minutes ago" the moment the verb landed. `OutcomeSignal` gains optional `completionGapMs` (gap since the last assistant message); the done narrative anchors on it, and the done-rule signal now reads "no user follow-up for X" — which is what `idleGapMs` actually measures.
- **Tone tokens misread common phrasings.** "are we done?" classified as *affirming* (the `done` token beat the question shape); "no rush, take your time" and "is the server still running?" classified as *correcting* (bare `no`/`still` tokens). Question shape now beats affirming tokens; `no` counts only when it leads the message and isn't a benign idiom; `still` counts only with a negative continuation ("still failing", "still does not work"). Correcting questions ("why is it still broken?") still read as correcting.
- **`npm run dev` counted as a verification.** The package-runner vocabulary matched `run <anything>`, so long-running servers (`dev`, `start`, `serve`, `watch`, `preview`, `storybook`) — which never terminate with a verifiable pass/fail — fed startup chatter into the pass/fail trend. Those script names are excluded; `npm run build` / `npm run test:unit` still count.

### Fixed — shell commands were invisible to the privileged-path rule

The privileged-path check only ran on tool *inputs* that carry a file path, so `Bash: cat ~/.ssh/id_rsa` — the single most likely way an agent actually touches `.ssh` — never fired, while the README claimed "any tool touching" these paths. Shell commands are now scanned token-by-token (via `tokenizeShellDeep`, so `bash -c "cat ~/.ssh/id_rsa"` and `--flag=~/.aws/credentials` forms are seen) against the same rule table. Only path-shaped tokens (containing a separator) are tested, so prose mentions of `.ssh` don't fire. The README scope note now matches actual coverage.
...
`decodeSlug`'s last-hyphen-segment rule is structurally lossy: a slug like `C--Workspace-example-learning-app` can't be split back into prefix + project because hyphens are both path-separator encodings and legal name characters, so every multi-word project decoded to its last segment (`app`, `orientation`, …). The transcript's first line carries the authoritative cwd, so naming now prefers `basename(cwd)` and falls back to slug-decode only when cwd is absent. Compounding it, `extractCwdFromFirstLine` was strictly first-line-only — but Claude Code transcripts open with `permission-mode` / `file-history-snapshot` lines before the first cwd-bearing message, so the cwd-first path never ran. It now scans up to 25 lines / 64 KB. A subagent-shaped cwd basename (`agent-<hex>`) is rejected so a project is never mislabelled as a tooling artifact.
...
Narratives like "Earlier in the window it made 12 changes to `C:\Users\demo\Dev\sample-app\src\dashboard\page.tsx` before going idle" wrapped onto two lines in a normal-width terminal — the v0.3.5 `NARRATIVE_MIN_HEIGHT` pad was the only thing keeping the detail box stable.
...
- **`repoRoot` / `--repo` was accepted but never used for drift detection.** Hardcoded prefixes (`/tmp/`, `/var/`, `~/`) meant a Write to `C:\Dev\other-project\` from a session rooted at `C:\Dev\sample-app\` didn't flag as drift. Now plumbed through `pulse()` → `classifyTrajectory` → `detectDrifts`. Cross-project writes correctly flag. Legacy hardcoded check still fires when `repoRoot` is undefined for backwards compat.
...
- **Cluster too shallow** — `top cluster C:/Dev covers 99% of activity` was useless because every project sat under that umbrella. The path clusterer now strips each event's `cwd` prefix before bucketing, so `C:/Dev/sample-app/utils/foo.py` clusters as `utils` instead of `C:/Dev`. Files outside the session's cwd retain absolute-path clustering (they're interesting in their own right). Files at the project root render as `(project root)` rather than `.` for narrative readability.
